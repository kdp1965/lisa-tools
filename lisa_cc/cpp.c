 ...
 * in this document: https://github.com/rui314/p16cc/wiki/cpp.algo.pdf
 */
...
static bool guarded(char *path) {
    ...
    define_obj_macro("__p16cc_include_guard", r ? cpp_token_one : cpp_token_zero);
    return r;
}
...
static void init_predefined_macros() {
    ...
    snprintf(str, sizeof(str)-1, "%s/include", gpToolPath);
    vec_push(std_include_path, strdup(str));

    define_special_macro("__LISA__", handle_date_macro);
    ...
    read_from_string("#include <" BUILD_DIR "/include/lisacc.h>");
}
