...
SourceLoc *func_source_loc = NULL;
SourceLoc *if_source_loc_stack[50];
int        if_source_stack_idx = 0;
...
Type *type_enum = &(Type){ KIND_ENUM, 2, 2, false };
...
static void mark_location() {
    ...
    if (tok->kind == TIDENT ||
        tok->kind == TSTRING ||
        tok->kind == TNUMBER)
      printf("Marking %s at line %d\n", tok->sval, tok->line);
    else if (tok->kind == TKEYWORD)
    {
       const char *pText;
       char    unknown[128];
       switch (tok->id) {
#define op(x, y)
#define keyword(id, kname, _) case id: { pText = kname; break; }
#include "keyword.inc"
#undef keyword
#undef op
       default:
          sprintf(unknown, "'%c'", tok->id);
          pText = unknown;
       }
       printf("Marking %s at line %d\n", pText, tok->line);
    }
    else
      printf("Marking line %d, id=%d\n", tok->line, tok->id);
#endif
}
...
char *make_label() {
    static int c = 0;
    return format("_L%d", c++);
}

static char *make_static_label(char *name) {
    static int c = 0;
    return format("_S%d.%s", c++, name);
}
...
static Node *ast_uop(int kind, Type *ty, Node *operand) {
    return make_ast(&(Node){ kind, copy_type(ty), .operand = operand });
}

static Node *ast_binop(Type *ty, int kind, Node *left, Node *right) {
    Node *r = make_ast(&(Node){ kind, copy_type(ty) });
    r->left = left;
    r->right = right;
    return r;
}
...
static Node *ast_inttype(Type *ty, long val) {
    return make_ast(&(Node){ AST_LITERAL, copy_type(ty), .ival = val });
}
...
static Node *ast_gvar(Type *ty, char *name) {
    Node *r = make_ast(&(Node){ AST_GVAR, copy_type(ty), .varname = name, .glabel = name });
...
static Node *ast_typedef(Type *ty, char *name) {
    Node *r = make_ast(&(Node){ AST_TYPEDEF, copy_type(ty) });
    map_put(env(), name, r);
    return r;
}
...
static Node *ast_funcall(Type *ftype, char *fname, Vector *args) {
    return make_ast(&(Node){
        .kind = AST_FUNCALL,
        .ty = copy_type(ftype->rettype),
        .fname = fname,
        .args = args,
        .ftype = copy_type(ftype) });
}

static Node *ast_funcdesg(Type *ty, char *fname) {
    return make_ast(&(Node){ AST_FUNCDESG, copy_type(ty), .fname = fname });
}
...
static Node *ast_init(Node *val, Type *totype, int off) {
    return make_ast(&(Node){ AST_INIT, .initval = val, .initoff = off, .initialized = false, .totype = copy_type(totype) });
}

static Node *ast_conv(Type *totype, Node *val) {
    return make_ast(&(Node){ AST_CONV, copy_type(totype), .operand = val });
}
...
static Node *ast_ternary(Type *ty, Node *cond, Node *then, Node *els) {
    return make_ast(&(Node){ AST_TERNARY, copy_type(ty), .cond = cond, .then = then, .els = els });
}
...
static Node *ast_struct_ref(Type *ty, Node *struc, char *name) {
    return make_ast(&(Node){ AST_STRUCT_REF, copy_type(ty), .struc = struc, .field = name });
}
...
static Type *make_numtype(int kind, bool usig) {
    ...
    else if (kind == KIND_FLOAT)   r->size = r->align = 2;
    else if (kind == KIND_DOUBLE)  r->size = r->align = 3;
    else if (kind == KIND_LDOUBLE) r->size = r->align = 4;
    else error("internal error");

    // LISA alignment is only 1 byte
    //r->align = 1;
    return r;
}

static Type* make_ptr_type(Type *ty) {
    return make_type(&(Type){ KIND_PTR, .ptr = copy_type(ty), .size = 2, .align = 1 });
}
...
bool is_chartype(Type *ty) {
    switch (ty->kind) {
    case KIND_BOOL: case KIND_CHAR:
        return true;
    default:
        return false;
    }
...
static void ensure_lvalue(Node *node) {
    ...
    case AST_LVAR: case AST_GVAR: case AST_DEREF: case AST_STRUCT_REF:
        return;
    default:
        error("lvalue expected, but got %s", node2s(node, -1));
    }
}

static void ensure_inttype(Node *node) {
    if (!is_inttype(node->ty))
        error("integer type expected, but got %s", node2s(node, -1));
}

static void ensure_arithtype(Node *node) {
    if (!is_arithtype(node->ty))
        error("arithmetic type expected, but got %s", node2s(node, -1));
}
...
int eval_intexpr(Node *node, Node **addr) {
    ...
    case AST_LITERAL:
        if (is_inttype(node->ty))
            return node->ival;
        error("Integer expression expected, but got %s", node2s(node, -1));
...
static Node *read_int(Token *tok) {
    ...
    // In lisa_cc, long and long long are the same size.
    bool base10 = (*s != '0');
    if (base10) {
...
static Node *read_generic() {
   ...
   if (!defaultexpr)
       errort(tok, "no matching generic selection for %s: %s", node2s(contexpr, -1), ty2s(contexpr->ty));
   return defaultexpr;
}
...
static Type *char_type(int enc) {
    ...
    case ENC_NONE:
    case ENC_WCHAR:
        return copy_type(type_int);
    case ENC_CHAR16:
        return copy_type(type_ushort);
    case ENC_CHAR32:
        return copy_type(type_uint);
    }
    ...
    for (;;) {
        if (next_token('(')) {
            ...
            if (t->kind != KIND_PTR && t->ptr->kind != KIND_FUNC)
                errort(tok, "function expected, but got %s", node2s(node, -1));
...
static Node *read_unary_addr() {
    ...
    if (operand->ty->issfr)
    {
      Token *tok = peek();
      char line[16];
      sprintf(line, "%d", tok->line);
      errorf(tok->file->name, line, "Can't take address of an SFR!\n");
    }
...
static Node *read_unary_deref(Token *tok) {
    ...
    if (operand->ty->kind != KIND_PTR)
        errort(tok, "pointer type expected, but got %s", node2s(operand, -1));
...
static Node *read_unary_bitnot(Token *tok) {
    ...
    expr = conv(expr);
    if (!is_inttype(expr->ty))
        errort(tok, "invalid use of ~: %s", node2s(expr, -1));
...
static Node *read_struct_field(Node *struc) {
    if (struc->ty->kind != KIND_STRUCT)
        error("struct expected, but got %s", node2s(struc, -1));
    Token *name = get();
    if (name->kind != TIDENT)
...
static Vector *read_rectype_fields_sub() {
    ...
    for (;;) {
        ...
        for (;;) {
            ...
            if (fieldtype->bitsize < 9)
            {
               fieldtype->kind = KIND_CHAR;
               fieldtype->size = 1;
               fieldtype->align = 1;
            }
...
static void skip_to_brace() {
    for (;;) {
        ...
        if (!ignore)
            return;
        warnt(tok, "excessive initializer: %s", node2s(ignore, -1));
        maybe_skip_comma();
    }
}
 ...
 *
 * In lisa_cc, delcarations are parsed by two functions: read_declarator
...
static Type *read_func_param(char **name, bool optional) {
    int sclass = 0;
    Type *basety = copy_type(type_int);
    if (is_type(peek())) {
...
static Type *read_decl_spec(int *rsclass) {
    ...
    for (;;) {
        ...
        case KACCUMULATOR: if (sclass) goto err; sclass = S_ACCUMULATOR; break;
        case KSFR:         if (sclass) goto err; sclass = S_SFR; break;
        case KISR:         if (sclass) goto err;
                              sclass = S_ISR; break;
        case KACCESS:      if (sclass) goto err; sclass = S_ACCESS; break;
        case KCONST:       break;
...
static void read_decl(Vector *block, bool isglobal) {
    ...
    for (;;) {
        ...
        ty->isregister = (sclass == S_REGISTER);
        ty->isaccumulator = (sclass == S_ACCUMULATOR);
        ty->issfr = (sclass == S_SFR);
        ty->isaccess = (sclass == S_ACCESS);
        if (sclass == S_TYPEDEF) {
            ast_typedef(ty, name);
...
static Node *read_funcdef() {
    ...
    functype->rettype->isisr = (sclass == S_ISR);
...
static Node *read_switch_stmt() {
    ...
    Vector *v = make_vector();
    Node *var;
    if (expr->kind == AST_LVAR || expr->kind == AST_GVAR)
        var = expr;
    else
    {
        var = ast_lvar(expr->ty, make_tempname());
...
static Node *read_goto_stmt() {
    if (next_token('*')) {
        ...
        if (expr->ty->kind != KIND_PTR)
            errort(tok, "pointer expected for computed goto, but got %s", node2s(expr, -1));
...
static Node *read_stmt() {
    ...
    if (tok->kind == TKEYWORD) {
        ...
        case KIF:       { if_source_loc_stack[if_source_stack_idx++] = source_loc; return read_if_stmt();}
