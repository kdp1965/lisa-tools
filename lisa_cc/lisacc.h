...
//
// Modified 2019 By Ken Pettit for the Pico16 Processor
...
#else
#define error(...)       errorf(current_file()->name, current_file()->line, NULL, __VA_ARGS__)
#define errort(tok, ...) errorf(current_file()->name, current_file()->line, token_pos(tok), __VA_ARGS__)
#define warn(...)        warnf(current_file()->name, current_file()->line, NULL, __VA_ARGS__)
#define warnt(tok, ...)  warnf(current_file()->name, current_file()->line, token_pos(tok), __VA_ARGS__)
#endif
...
#else
noreturn void errorf(char *file, int ine, char *pos, char *fmt, ...);
